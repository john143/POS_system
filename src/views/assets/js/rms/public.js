...
const api_url = 'http://rslivetv.com/api/';
var current_time = '';
var stop_time = '';
...
$(document).ready(function(){
	...
	$('#sidebar_menu').load("elements/sidebar.html")
	...
	$(document).on('click', '.logout_me', function(){
		...
		if(status == 'done'){
			/*$.ajax({
				url: 'http://digitalwebdevelopers.net/JBJ/api/login',
				type:'get',
				beforeSend:function(){
					$('.auto_loader').show();
				},
	...
	$(document).on("keypress", ".number", function (event) {
	  ...
	  setInterval(function(){
		  //var temp_device_id = $.trim($('.device_id').val());
		  $.ajax({ 
				url: api_url+'channels',
				type:'get',
				data:{device_id:tt_device},
				success:function(resp){
					console.log('device_idgg: '+resp.device_id);
					if(resp.device_id == "false"){
						var notification = new Notification("RS Live TV", {body: "This account is accessed on other device."});
						setTimeout(function() {
							notification.close();
							window.location.href = './users/login.html' ;
						}, 5000);
						//alert('This account is accessed on other device. Redirecting ...');
...
function afterHeaderLoaded(){
	var initInfo = ipcRenderer.sendSync('initView', []);
	$('.head_shop_name').text(initInfo.bussiness_name);
	$('.head_shop_name2').text(initInfo.bussiness_name);
	if(!initInfo.logo){
		$('.head_shop_img').prop('src', '../assets/img/default_shop.png');
		$('.head_shop_img2').prop('src', '../assets/img/default_shop.png');
	}else{
