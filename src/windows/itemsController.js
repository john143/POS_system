...
class users {
	...
	addItem(arg, callback) {
		var current_dt = getDateTime();
		//console.log('cat cont : ' + JSON.stringify(arg))
		var frm_data = arg;
		db.get("SELECT * FROM item_master WHERE code = '" + frm_data.code + "' OR name = '" + frm_data.item_name + "'  ", function (err, row) {
			if (err) {
			...
			if (!row) {
				...
				db.run("INSERT INTO item_master (cat_id,code,name,status,pickup_price,delivery_price,eat_in_price,cost_price,stock,min_stock,desc,contain,extra,created_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)", [frm_data.cat_id,frm_data.code,frm_data.item_name,frm_data.status, frm_data.pick_up_price,frm_data.delivery_price,frm_data.eat_in_price,frm_data.cost_price,frm_data.stock,frm_data.min_stock,frm_data.desc,frm_data.contain_idzs,frm_data.extra_idzs, current_dt], function (err) {
					if (err) {
	...
	getItems(arg, callback) {
		db.all("SELECT * FROM item_master ORDER BY tid ASC", function (err, row) {
			if (err) {
